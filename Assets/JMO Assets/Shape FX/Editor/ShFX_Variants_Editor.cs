...
namespace ShapeFX
{
	...
	public class ShFX_Variants_Editor : Editor
	{
		...
		private ShFX_Variants.EffectDensity CreateEffectDensity(ParticleSystem particleSystem)
		{
			...
			SerializedProperty emissionRateProp = new SerializedObject(effectDensity.particleSystem).FindProperty("EmissionModule.rate.scalar");
...
//					int frameIndex = shapeIndex*3 + styleIndex;
			...
			static private EffectScales GetParticleSystemScaleValues(ParticleSystem ps, bool isRoot)
			{
				...
				effectScales.gravityModifier = so.FindProperty("InitialModule.gravityModifier").floatValue;
				
				bool distanceEmission = so.FindProperty("EmissionModule.m_Type").intValue == 1;
				effectScales.emissionRate = distanceEmission ? so.FindProperty("EmissionModule.rate.scalar").floatValue : 0f;
			...
			private void SetParticleSystemScaleValues(EffectScales effectScales, float scale)
			{
				...
				so.FindProperty("InitialModule.gravityModifier").floatValue = effectScales.gravityModifier * scale;
				
				bool distanceEmission = so.FindProperty("EmissionModule.m_Type").intValue == 1;
				if(distanceEmission)
				{
					so.FindProperty("EmissionModule.rate.scalar").floatValue = effectScales.emissionRate * scale;
				}
		...
		private bool ShouldUpdateEmission(ShFX_Variants.EffectDensity effectDensity, out bool emissionChanged)
		{
			...
			SerializedProperty emissionRateProp = particleSystemSO.FindProperty("EmissionModule.rate.scalar");
